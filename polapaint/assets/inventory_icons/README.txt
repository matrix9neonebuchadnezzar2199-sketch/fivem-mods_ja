...
B. `ox_inventory/web/images/` へコピーする場合（ox 2.4x）: `items.lua` では **`client.image = 'polaroid_camera.png'`**（.png 付き）にするか **`client.image` を省略**する。`polaroid_camera` だけだと Web が `.png` を付けず表示されないことがあります。

...
- **Transform パネル（M2-e）**: フッター下段に展開し、**位置／回転それぞれのステップチップ**（m または °）と **± ボタン**でドラフト編集し、**「適用」**で `tecton:op:update` 送信まで保留する。**回転は送信時に 0〜360° に正規化**し、極端な数値は `rangeError` でブロック。**「キャンセル」**は未適用の入力のみロールバック（サーバ通信なし）。**「削除」**は確認ダイアログ経由で `deleteObject`。**ワールド選択**はチャット **`/tecPick`**（カプセルレイキャスト、`spawnedHandles` 逆引き）で行い、必要ならビルダーを開く。

...
- **`PropList`** は `react-window` の **`Grid`** で仮想スクロール（サムネ付きカード）。列数は **最大 4 列**（最小幅 **140px** 相当で `floor(listWidth/140)` により狭い幅では 1〜3 列に縮退）。`Grid` の `style.width` は **`columnWidth * columnCount`** とし、端数 px での横スクロールを避ける。**ビルダーパネル幅**は `calc(100vw - 2rem)`（左寄せオーバーレイのパディング分を除いた全幅）。

...
`config/config.lua`：DB接続前提、シーン既定値、オートセーブ間隔、Undo保持件数（既定500）、UI色テーマ等。ビルダー開閉は **`/tecton` / `/tec`** のチャットコマンドのみ（ホットキーは使わない）。
...
現行実装ではビルダーは **`/tecton` / `/tec`** のみ。将来のショートカット（`Ctrl+Z` Undo 等）は `spec` 全体設計に沿って追加予定。
...
M2 UI完全版：ツリー・検索・フィルタ・最近使用・お気に入り・サムネ・カラーパレット・トランスフォーム・スナップ全種・ショートカット。
  - **M2-e（Transform パネル仕上げ）**: ✅ ステップ ±・位置／回転ステップ切替・回転正規化・入力レンジ検証・編集キャンセル（ロールバック）・削除確認・レイキャスト選択（**`/tecPick`** のみ）。実機シナリオ検証は開発日記を参照。

const ResourceName =
    typeof GetParentResourceName === "function" ? GetParentResourceName() : "qb-storerobbery";
...
Padlock.Close = function () {
    ...
    $.post(`https://${ResourceName}/PadLockClose`);
};
...
Keypad.Close = function (data) {
    ...
    $.post(`https://${ResourceName}/PadLockClose`);
    if (data.error != null) {
        $.post(`https://${ResourceName}/CombinationFail`);
    }
};
...
function submitForm(e) {
    $("#keypad").css("display", "none");
    $.post(
        `https://${ResourceName}/TryCombination`,
        JSON.stringify({
            combination: e.value,
        })
    );
    ...
    for (let i = 0; i < comboArr.length; ++i) {
        if (!$(".num" + (i + 1)).hasClass("found")) {
            if (angle > (comboArr[i] - numOffset) * tickAngle && angle < (comboArr[i] + numOffset) * tickAngle) {
                ...
                // on unlock
                $.post(`https://${ResourceName}/callcops`);
                if (i == comboArr.length - 1) {
                    // unlock :)
                    $.post(`https://${ResourceName}/PadLockSuccess`);
                    Padlock.Close();
                }
            }
            break;

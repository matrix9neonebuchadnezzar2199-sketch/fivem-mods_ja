...
-- =====================================================================
-- State reset helper (FORK ADDITION)
--
-- Multiple cancellation paths in the original code forgot to reset
-- one or more of: currentRegister, currentSafe, copsCalled. This
-- caused the "next robbery uses wrong register coordinates" and
-- "police are not alerted on the next robbery" bugs.
-- =====================================================================
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        local registerId = currentRegister
        lockpick(false)
        TriggerServerEvent('qb-storerobbery:server:setRegisterStatus', registerId)
        ...
        QBCore.Functions.Progressbar('search_register', Lang:t('text.emptying_the_register'), lockpickTime, false, true, {
            ...
            TriggerServerEvent('qb-storerobbery:server:takeMoney', registerId, true)
            ResetRobberyState() -- FORK FIX: clear copsCalled / positions after successful robbery
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            QBCore.Functions.Notify(Lang:t('error.process_canceled'), 'error')
            ResetRobberyState() -- FORK FIX
        end)
        CreateThread(function()
...
function LockpickDoorAnim(time)
    ...
    CreateThread(function()
        while openingDoor do
            ...
            Wait(2000)
            time = time - 2
            TriggerServerEvent('qb-storerobbery:server:takeMoney', registerId, false)
            if time <= 0 then
                openingDoor = false
...
RegisterNUICallback('PadLockClose', function(_, cb)
    SetNuiFocus(false, false)
    ResetRobberyState() -- FORK FIX (was only resetting copsCalled)
    cb('ok')
end)
...
RegisterNUICallback('fail', function(_, cb)
    ...
    lockpick(false)
    SetNuiFocus(false, false) -- 二重保険（NUI 側でフォーカスが残る場合への対策）
    ResetRobberyState() -- FORK FIX: reset state on lockpick failure
    cb('ok')
end)

RegisterNUICallback('exit', function(_, cb)
    lockpick(false)
    SetNuiFocus(false, false) -- 二重保険
    ResetRobberyState() -- FORK FIX
    cb('ok')
end)
    ...
    QBCore.Functions.TriggerCallback('qb-storerobbery:server:isCombinationRight', function(combination)
        if tonumber(data.combination) ~= nil then
            if tonumber(data.combination) == combination then
                ...
                ResetRobberyState() -- FORK FIX: reset all state, not just currentSafe
            end
        end
        cb('ok')
    end, currentSafe)
...
-- NUI フォーカス解放漏れ時の緊急脱出（Backspace = 202。ESC はポーズメニューと競合しやすいため未使用）
CreateThread(function()
    while true do
        if currentRegister ~= 0 or currentSafe ~= 0 then
            Wait(50)

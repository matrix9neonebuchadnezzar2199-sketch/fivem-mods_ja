...
ゲート転送（Cayo Perico 住宅街 5 ヶ所からランダム選出）
  ↓
[Survival 即開始] ← 既定で Config.Arena.enabled=false
  ↓ firstSpawnMs (20 秒) 後に最初のスポーン
自由探索フェーズ（ルート回収・脱出ポイント選択）
  ・各メンバー周辺 30〜150m に 20 秒後 → 以降 3 分ごとに 3 体スポーン
  ・サバイバル感を維持しながら、ルートと脱出ルート選択の自由度
  ↓
個別脱出（5 ヶ所のいずれか）or タイムアウト
```

### 1-B. ウェーブ制廃止（INSTRUCTION-021 確定運用）

マスター実機検証の結果、**初期位置が塀に囲まれているため最初のウェーブのゾンビが入って来られない問題**が判明。3 ウェーブ制を廃止し、`Config.Arena.enabled = false` を既定とした **「最初から探索モード・ランダム湧き」** 運用に確定。

- 既定: **`Config.Arena.enabled = false`**（3 ウェーブ無効、即サバイバル）
- アリーナ実装は **コード残置**（将来の特定難易度・ボス戦・チュートリアル等で再利用余地）
- `Config.Arena.enabled = true` に戻せば INSTRUCTION-011 の 3 ウェーブ動作が復活する

### 1-C. `firstSpawnMs` の追加

任務開始直後（フェード後）にすぐゾンビが湧くと演出として唐突なため、**最初の 1 サイクルだけ `firstSpawnMs = 20000`（20 秒）** を使う。以降は通常の `intervalMs = 180000`（3 分）。プレイヤーが武器確認・周囲確認する時間を確保した上で、すぐ脅威が来る設計。
...
Config.Survival = {
    enabled = true,
    firstSpawnMs = 20 * 1000,           -- 開始から最初のスポーンまで（短め）

...
- **サイト・ナイン MAP（INSTRUCTION-020 v5 確定運用）**: **GTA V バニラ同梱の Cayo Perico** を採用。クライアント起動時に **`SetIslandEnabled('HeistIsland', true)` 常時 ON**。**`EnableMpDlcMaps` は呼ばない**（v3 で ESC マップ崩壊の主犯と判明）。bucket 0 のプレイヤーには海上に Cayo Perico が遠景表示、地続きアクセス不可（海上独立島）。bucket 分離は **演出のみ**（天気・時間・タイムサイクル・街灯）。座標基準は住宅街 5 ヶ所からランダム選出。撤回履歴: v1 The Apocalypse Project（LS ymap で bucket 分離不可）／ v2 North Yankton（Prologue ジオメトリ未完成）／ v3 Cayo Perico+EnableMpDlcMaps（ESC マップ崩壊）／ v4 Sandy Shores（設計の本質を失う）。

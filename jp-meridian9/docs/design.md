...
- **サイト・ナイン MAP（INSTRUCTION-020 v4 確定運用）**: **LS 内 Sandy Shores 北部の Meridian-9 隔離区域**（追加 MAP MOD なし）。MAP 切替系ネイティブ（`SetIslandEnabled` / `EnableMpDlcMaps`）は **一切呼ばない**。サイト・ナイン感は演出（雷雨・夜 22 時・青いフィルター・街灯消灯）と RP 設定（「許可なき侵入は契約違反でヴェガが対処」）で担保。`Config.SiteNine.island = 'none'`。撤回履歴: v1 The Apocalypse Project（LS ymap で bucket 分離不可）／ v2 North Yankton（Prologue 専用ジオメトリ未完成）／ v3 Cayo Perico（`SetIslandEnabled` 常時 ON 運用で GTA V クライアントの ESC マップ画面崩壊・LS 描画失敗を引き起こすため撤退）。

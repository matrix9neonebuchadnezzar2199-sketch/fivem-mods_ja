...
var WordCrack = (function() {
    ...
    function createDisplay() {
        ...
        var $instruction = $('<div class="word-crack-instruction">' + wordLength + '文字の単語を推測。緑=正解・黄=位置違い・赤=含まれない</div>');
        ...
        $buttons.append('<button class="word-crack-btn guess-btn" id="guess-btn">推測</button>');
        $buttons.append('<button class="word-crack-btn delete-btn" id="word-delete-btn">削除</button>');
        $inputArea.append($buttons);
        
        $display.append($inputArea);
        
        var $history = $('<div class="word-crack-history"></div>');
        $history.append('<div class="word-crack-history-title">試行: <span id="word-attempts">0</span>/' + maxAttempts + '</div>');
    ...
    function endGame(success) {
        ...
        var $result = $('<div class="word-crack-result ' + (success ? 'success' : 'failure') + '">' + 
            (success ? '単語突破' : 'ロックアウト - 単語: ' + correctWord) + '</div>');

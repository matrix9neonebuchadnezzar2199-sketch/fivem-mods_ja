...
function updateSymbolSearchUI() {
    ...
    $('#symbol-search-message').text('対象シーケンスの先頭を見つけてください');
}
...
function endSymbolSearchGame(success, reason) {
    ...
    if (success) {
            $('#symbol-search-message').text('シーケンス発見！アクセス許可。');
    } else {
        switch (reason) {
            case 'timeout':
                $('#symbol-search-message').text('時間切れ！アクセス拒否。');
                break;
            case 'wrong_selection':
                $('#symbol-search-message').text('位置が違います！アクセス拒否。');
                break;
            default:
                $('#symbol-search-message').text('アクセス拒否！');
        }
    }

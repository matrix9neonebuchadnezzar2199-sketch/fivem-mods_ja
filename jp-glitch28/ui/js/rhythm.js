...
function setupRhythmGame(config) {
    ...
    $('#rhythm-key-hint').text('キー ' + rhythmKeys.join(', ') + ' でノートを叩いてください');
...
function moveNotes() {
    ...
    for (let i = rhythmNotes.length - 1; i >= 0; i--) {
        ...
        if (note.position > hitZonePos + 50) {
            ...
            $('#rhythm-message').text(`許容ミス ${missedNotes}/${maxMissedNotes} ノート`);
...
function showFeedback(lane, timing) {
    ...
    const timingJp = { perfect: '完璧', great: 'グレート', good: 'グッド', okay: 'オッケー', miss: 'ミス' };
    feedback.text(timingJp[timing] || timing.toUpperCase());
...
function resetRhythmGame() {
    ...
    $('#rhythm-message').text('ビートに合わせてノートを叩いてください');
}
...
function stopRhythmGame(success) {
    ...
    if (success) {
        $('#rhythm-message').text('同期完了！回路が安定しました。');
        playSoundSafe('sound-success');
    } else {
        if (missedNotes >= maxMissedNotes) {
            $('#rhythm-message').text('同期失敗！ミスが多すぎます。');
        } else if (wrongKeyCount >= maxWrongKeys) {
            $('#rhythm-message').text('同期失敗！誤入力が多すぎます。');
        } else {
            $('#rhythm-message').text('同期失敗！回路が過負荷です。');
        }

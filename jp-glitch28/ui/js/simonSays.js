...
window.simonSaysGame = (function () {
    ...
    function startRound() {
        ...
        $('#ss-round').text('ラウンド ' + roundNum + ' / ' + maxRounds);
        $('#ss-status').text('よく見て…').removeClass('ss-ok ss-fail');
        setInputEnabled(false);
        ...
        showTimeout = setTimeout(() => {
            playSequence(() => {
                // Player's turn
                $('#ss-status').text('あなたの番！').addClass('ss-ok');
                startInputTimer();
            });
        }, 600);
    }
    ...
    function startInputTimer() {
        ...
        timerInt = setInterval(() => {
            ...
            $('#ss-time-text').text(Math.ceil(timeLeft) + '秒');
            if (timeLeft <= 0) {
                clearInterval(timerInt);
                onFail();
            }
    ...
    function onRoundWin() {
        ...
        setInputEnabled(false);
        phase = 'idle';
        $('#ss-status').text('正解！ ✓').addClass('ss-ok');
    ...
    function onFail() {
        ...
        $('#ss-status').text('不正解！').removeClass('ss-ok').addClass('ss-fail');

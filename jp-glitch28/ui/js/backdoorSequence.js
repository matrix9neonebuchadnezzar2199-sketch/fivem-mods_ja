...
(function() {
        ...
        if (key === expectedKey) {
            ...
            if (pressedKeys.length === stageKeys.length) {
                ...
                if (currentStage >= sequenceConfig.totalStages) {
                    ...
                    $('#seq-message').text('ステージ完了！次のシーケンスを開始…');
                    setTimeout(() => {
                        $('#seq-message').text('暗号を破るためシーケンスを入力してください');
                        generateNewSequence();
                    }, 1000);
                }
            }
        } else {
            onSequenceFailure('キー誤り — シーケンス失敗');
        }
    }
    ...
    function onSequenceSuccess() {
        ...
        stopSequenceTimer();
        $('#seq-message').text('シーケンス完了！バックドアを開きました。');
    ...
    function onSequenceFailure(reason) {
        sequenceActive = false;
        stopSequenceTimer();
        $('#seq-message').text(reason || 'シーケンス失敗！');

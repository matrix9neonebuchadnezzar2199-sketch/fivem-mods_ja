...
function updateNumberedSequenceUI() {
    ...
    $('#numbered-sequence-message').text(`ラウンド ${numberedSequenceGameState.currentRound + 1} - 今回の誤入力: ${numberedSequenceGameState.wrongPresses}/${numberedSequenceGameState.config.maxWrongPresses}`);
}
...
function startNumberedSequenceRound() {
    ...
    updateNumberedSequenceUI();
    $('#numbered-sequence-message').text('数字の順序を記憶してください');
...
function hideNumbersStartGame() {
    ...
    $('#numbered-sequence-message').text(`マスを数字順にクリック (1〜${numberedSequenceGameState.config.sequenceLength})`);
...
function startGuessTimer() {
    ...
    numberedSequenceGameState.guessTimer = setTimeout(() => {
        if (numberedSequenceGameState.gameActive) {
            ...
            numberedSequenceGameState.gameActive = false;
            numberedSequenceGameState.showingPattern = false;
            $('#numbered-sequence-message').text('時間切れ！ゲーム失敗。');
...
function handleNumberedSequenceSquareClick() {
    ...
    if (squareIndex === expectedSquare.index) {
        ...
        if (numberedSequenceGameState.currentExpectedNumber > numberedSequenceGameState.config.sequenceLength) {
            ...
            $('#numbered-sequence-message').text('シーケンス正解！');
        ...
        $('#numbered-sequence-message').text(`違うマスです！ 期待する数字 ${numberedSequenceGameState.currentExpectedNumber} (${numberedSequenceGameState.wrongPresses}/${numberedSequenceGameState.config.maxWrongPresses})`);
        ...
        if (numberedSequenceGameState.wrongPresses >= numberedSequenceGameState.config.maxWrongPresses) {
            ...
            numberedSequenceGameState.gameActive = false;
            numberedSequenceGameState.showingPattern = false;
            $('#numbered-sequence-message').text('誤入力が多すぎます！ゲーム失敗。');
            playNumberedSequenceSound('failure');
            setTimeout(() => {
                endNumberedSequenceGame(false);
            }, 1500);
...
function endNumberedSequenceGame(success) {
    ...
    if (success) {
        $('#numbered-sequence-message').text('数列テスト成功！');
        playNumberedSequenceSound('success');
    } else {
        const currentMessage = $('#numbered-sequence-message').text();
        if (!currentMessage.includes('失敗') && !currentMessage.includes('時間切れ')) {
            $('#numbered-sequence-message').text('数列テスト失敗！');
            playNumberedSequenceSound('failure');
        }
    }

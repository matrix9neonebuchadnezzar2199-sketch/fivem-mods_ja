...
function updateMemoryUI() {
    ...
    $('#memory-message').text(`パターンを記憶してください（誤入力: ${memoryGameState.wrongPresses}/${memoryGameState.config.maxWrongPresses}）`);
}
...
function hidePattern() {
    ...
    $('#memory-message').text(`光っていたマスをクリックしてください（誤入力: ${memoryGameState.wrongPresses}/${memoryGameState.config.maxWrongPresses}）`);
...
function handleMemorySquareClick() {
    ...
    if (memoryGameState.selectedSquares.includes(squareIndex)) {
        ...
        $('#memory-message').text(`誤入力！ (${memoryGameState.wrongPresses}/${memoryGameState.config.maxWrongPresses})`);
...
function endMemoryGame(success) {
    ...
    if (success) {
        $('#memory-message').text('メモリーテスト成功！');
        playMemorySound('success');
    } else {
        $('#memory-message').text('メモリーテスト失敗！');

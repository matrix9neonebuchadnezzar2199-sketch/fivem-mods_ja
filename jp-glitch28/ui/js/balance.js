...
let balanceGame = {
    ...
    start: function(config) {
        ...
        $('.balance-status').text('Q または E を押す').removeClass('danger warning').addClass('ready');
        $('.balance-instruction').text('Q または E で安定化を開始 — 針のバランスを保ってください');
    },
    ...
    beginGame: function() {
        ...
        $('.balance-instruction').text('Q/E で針のバランスを取り、赤ゾーンに落とさないでください！');
        $('.balance-status').text('安定').removeClass('ready').addClass('safe');
    ...
    startGameLoop: function() {
        ...
        this.gameInterval = setInterval(function() {
            ...
            if (zone === 'red') {
                ...
                $('.balance-status').text('危険！').removeClass('safe warning ready').addClass('danger');
                ...
                $('.balance-status').text('注意').removeClass('safe danger ready').addClass('warning');
            } else {
                self.dangerTime = Math.max(0, self.dangerTime - frameRate);
                $('.balance-status').text('安定').removeClass('danger warning ready').addClass('safe');
            }
    ...
    endGame: function(success) {
        ...
        $result.text(success ? '安定化成功！' : '不安定化！');

...
function updateSequenceMemoryUI() {
    ...
    $('#sequence-memory-message').text(`シーケンスを記憶してください（誤入力: ${sequenceMemoryGameState.wrongPresses}/${sequenceMemoryGameState.config.maxWrongPresses}）`);
}
...
function startSequenceMemoryRound() {
    ...
    updateSequenceMemoryUI();
    $('#sequence-memory-message').text('シーケンスを見てください');
...
function showSequencePattern() {
    ...
    function showNextSquare() {
        if (index >= sequenceMemoryGameState.sequence.length) {
            ...
            $('#sequence-memory-message').text('シーケンスを繰り返してください');
            return;
        }

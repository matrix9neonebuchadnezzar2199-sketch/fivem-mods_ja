...
var CodeCrack = (function() {
    ...
    function createDisplay() {
        ...
        var $instruction = $('<div class="code-crack-instruction">数字を入力してPINを解析。緑=正解・黄=位置違い・赤=含まれない</div>');
        ...
        $buttons.append('<button class="code-crack-btn crack-btn" id="crack-btn">解析</button>');
        $buttons.append('<button class="code-crack-btn delete-btn" id="delete-btn">削除</button>');
        $inputArea.append($buttons);
        
        $display.append($inputArea);
        
        var $history = $('<div class="code-crack-history"></div>');
        $history.append('<div class="code-crack-history-title">試行: <span id="crack-attempts">0</span>/' + maxAttempts + '</div>');
    ...
    function endGame(success) {
        ...
        var $result = $('<div class="code-crack-result ' + (success ? 'success' : 'failure') + '">' + 
            (success ? 'コード突破' : 'ロックアウト - PIN: ' + correctPin.join('')) + '</div>');

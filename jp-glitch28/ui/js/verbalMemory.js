...
function showNextWord() {
    ...
    setTimeout(() => {
        ...
        $('#verbal-memory-message').text("既出なら「見た」、初出なら「新規」をクリック");
...
function handleTimeExpired() {
    ...
    $('#verbal-memory-message').text("時間切れ！遅すぎます…");
...
function endVerbalMemoryGame(success) {
    ...
    if (success) {
        $('#verbal-memory-message').text(`完璧！ 全 ${verbalMemoryGameState.config.wordsToShow} 語を完了しました！`);
    } else {
        $('#verbal-memory-message').text(`ゲームオーバー！ 最終スコア: ${verbalMemoryGameState.score}`);
    }

...
**サーバー負荷はほぼかかりません。** このリソースに **`server_scripts` はありません**。進行・図鑑・表示位置などのデータは **クライアント側（KVS）のみ** に保存し、**サーバーへは送信しません**。ゲームロジックと NUI はクライアントで完結するため、プレイヤー数が増えてもサーバー CPU・DB・イベント負荷は実質増えません。
...
クライアント KVS キー `losmon_v1` に JSON で保存します（図鑑・ミニ表示位置含む）。サーバー側スクリプトはなく、上記のとおりデータはクライアントのみです。

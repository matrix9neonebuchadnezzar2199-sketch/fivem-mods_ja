return {
    -- デバッグ（PolyZone 可視化 / ログ）
    debug = false,

    -- 表示言語（locales/<lang>.json）
    -- 'ja' 推奨
    locale = 'ja',
    ...
    depot = {
        ...
        -- 半径(m)
        interactRadius = 8.0,
        pedModel = 'a_m_m_indian_01', -- 受付NPC モデル
        label = 'タクシー受付',       -- ox_target ラベル
    },
    ...
    -- ======== 社用車 ========
    companyVehicles = {
        { model = 'taxi', label = 'タクシー (標準)' },
        -- { model = 'dynasty', label = 'タクシー (クラシック)' },
    },
    -- qbx_taxijob と同一（11 箇所）
    cabSpawns = {
    ...
    personalVehicleWhitelist = {},

    -- ======== メーター ========
    meter = {
        defaultPrice = 125.0, -- 1マイルあたり
        startingPrice = 0,
        useGpsPrice = true, -- true: GPS（乗車地点→降車地点）/ false: 走行距離
    },

    -- qbx_taxijob 相当: PolyZone
    pzLocations = {
        takeLocations = {
    ...
    -- ======== 社用車：離反ペナルティ ========
    company = {
        warnDistance = 50.0, -- 警告
        endDistance = 100.0, -- 勤務終了+削除
    },

    -- ======== 勤務制限 ========
    requireJob = false,       -- true: 指定ジョブ必須
    requiredJobName = 'taxi', -- requireJob true の場合に使う

    -- ======== 互換: qbx 由来の料金乱数/アイテム（サーバーで使用） ========
    rewards = {
        -- 乱数上乗せ（小銭）を有効化（qbx 相当の挙動）
        enableRandomExtraCash = true,
        -- cryptostick ボーナス
        enableCryptostick = true,
        cryptostickChance = 25, -- %
    }
}

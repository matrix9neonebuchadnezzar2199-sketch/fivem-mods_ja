...
if GetCurrentResourceName() ~= "pls_jobsystem" then
    print("^1[pls_jobsystem] エラー: リソース名は 'pls_jobsystem' でなければなりません。現在: '" ..
    GetCurrentResourceName() .. "'。クライアントは起動しません。^0")
    return
end
...
-- GetLocale() は config.lua（shared）で定義
...
local function DoRaycast()
    ...
    NUINotify({
        title = (Locales[GetLocale()] or Locales["en"]).select_point,
        description = "[ E ]",
        type = "inform"
    })
    ...
    if data.craftingData then
        ...
        if hasAllItems then
            ...
            if lib.progressCircle({
                    duration = 10000,
                    label = string.format(locale.crafting_progress, itemLabel),
                    position = 'bottom',
                    useWhileDead = false,
                    canCancel = true,
                    disable = {

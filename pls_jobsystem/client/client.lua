...
local function Loc()
    return Locales[GetLocale()] or Locales["en"]
end
...
function PlaceProp(modelName)
    ...
    while not HasModelLoaded(model) do
        ...
        if timeout > 5000 then
            ...
            NUINotify({
                title = Loc().error,
                description = string.format(Loc().model_not_found, modelName),
                type = "error"
            })
            return false
        end
    ...
    local function buildHint()
        ...
        return string.format(Loc().gizmo_hint, axis, axis, axis, heightOff, step)
    end
...
local function NormalizeICStation(station)
    ...
    if station.resultItem and station.resultItem ~= "" then
        local recipe = {
            id              = station.id .. "_r0",
            label           = station.label or Loc().label_recipe,
        ...
        CreateThread(function()
            while isInteractiveCrafting do
                ...
                if curProp and hit and entityHit == curProp then
                    lib.showTextUI(string.format(Loc().ic_prompt_use, recipe.ingredients[currentStep].itemName))
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if ok then
                            ...
                            NUINotify({
                                title = Loc().error,
                                description = string.format(Loc().missing_ingredient, ingItem),
                                type = "error"
                            })
                            break
                        end
...
function StartInteractiveCrafting(station, jobName)
    ...
    if #recipes == 0 then
        NUINotify({ title = Loc().error, description = Loc().no_recipes, type = "error" })
        return
    end
          ...
          options = {{
              ...
              onSelect = function()
                ...
                if jobname == job.job then
                  ...
                  local locale = Loc()
                  NUINotify({ title = locale.error, description = locale.not_your_job, type = "error" })
                end
              end
          }},
          debug = false,
        ...
        options = {
            {
                ...
                onSelect = function(data)
                  ...
                  if jobname == job.job then
                    ...
                    NUINotify({
                      title = locale.error,
                      description = locale.not_your_job,
                      type = "error"
                    })
                  end
                end
        ...
        options = {
            {
                name = 'bell',
                icon = 'fa-solid fa-circle',
                label = Loc().label_alarm,
                onSelect = function(data)
                  local jobname = BRIDGE.GetPlayerJob()
                  if jobname == job.job then
                    OpenNUI("confirm", {
                      header = Loc().label_call_police,
                      content = Loc().call_police_confirm,

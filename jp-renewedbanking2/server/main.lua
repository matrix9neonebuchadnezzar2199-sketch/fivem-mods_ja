--[[
    Renewed-Banking / server/main.lua
    原作: uShifty (Renewed-Scripts) — CC BY-NC-SA 4.0
    原リポジトリ: https://github.com/Renewed-Scripts/Renewed-Banking
    日本語化派生: jp-renewedbanking2（コメント和訳のみ。処理ロジックは原作に準拠）
]]
...
--[[ changeAccountName export:
    原作コメント — 高信頼バックエンドでのみ利用すること（サーバー専用 export のため悪用に注意）
    Original: Should only use this on very secure backends... server side ONLY export. ]]

--- キャッシュ上のジョブ口座を取得する（存在しなければ nil）
---@param jobName string ジョブ ID（例: police）
---@return table|nil account 口座テーブルまたは nil
...
--[[ 組織（ジョブ）用の共有口座を作成
    @param job table job.name = 一意 ID、job.label = 表示名
    @param initialBalance number? 初期残高（省略時 0）
    @return table 口座テーブル。検証／DB 失敗時は error を投げ得る
    (EmmyLua 英語原文の意味は上記と同等)]]
...
-- 近距離の現金譲渡（原作の help 文字列は英語のまま／ox_lib コマンドヘルプ用）

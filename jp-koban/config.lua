-- jp-koban 共有設定（運営者向け。数値・座標の説明は日本語コメント参照）
Config = {}

-- ジョブ制限（Qbox: job.name と一致するプレイヤーだけ受注・報告可）
Config.RequiredJob = 'police'

-- 完了報酬（全箇所巡回後に NPC で報告したときだけ一括付与。DB 不使用）
Config.CompletionBonus5 = 2000 -- 5 箇所コース
Config.CompletionBonus10 = 5000 -- 10 箇所コース

-- 選べるコース（箇所数）
Config.PatrolCounts = { 5, 10 }

-- 受付 NPC（ミッションロー警察署前）
-- NPC の足元微調整（+ で足を上げる＝床から沈みを防ぐ。- で沈める）。MLO では 0.05 ～ 0.15 程度
Config.JobPedZOffset = 0.1
Config.JobPedModel = 's_m_y_cop_01'
Config.JobPedCoords = vector4(441.68, -981.14, 30.69, 186.72)
Config.JobPedScenario = 'WORLD_HUMAN_CLIPBOARD'

-- 受付 NPC 付近の常時ブリップ
Config.NpcBlipSprite = 60
Config.NpcBlipColor = 29
Config.NpcBlipScale = 0.8
Config.NpcBlipLabel = '警察署（巡回受付）'

-- 巡回先ブリップ（1 箇所ずつ表示）
Config.PatrolBlipSprite = 1
Config.PatrolBlipColor = 38
Config.PatrolBlipScale = 0.9
Config.PatrolBlipLabel = '巡回ポイント'

-- インタラクト
Config.CheckDuration = 5 -- 巡回確認プログレスバー（秒）
Config.InteractRadius = 2.0 -- 地点・NPC 到達とみなす半径（m）
Config.MarkerDrawRadius = 60.0 -- 巡回用マーカーを描画し始める距離（XY、メートル）
-- 円形マーカー用（円盤: DrawMarker タイプ1 シリンダ高さ薄め）。Z は巡回地点座標
Config.OuterRingDiameter = 8.0 -- エリア可視用の大きい円（m 直径目安）
Config.PatrolRingZOffset = 0.1 -- 地面と重なり防止のZ加算

-- 途中キャンセル用コマンド
Config.CancelCommand = 'patrol'

-- 巡回候補座標（jp-LetterCarrier Config.DeliveryLocations と同一）
Config.PatrolLocations = {
